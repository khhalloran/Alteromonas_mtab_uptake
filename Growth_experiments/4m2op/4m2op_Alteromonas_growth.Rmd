---
title: "4m2op_Alteromonas_growth"
author: "KHH"
date: "4/10/2024"
...
Analyze growth of Alteromonas macleodii on 4-methyl-2-oxopentanoic acid ± pyruvate. 
...
path_for_saving_plots <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/04.2024_reanalysis/4m2op/Figures/"

  
##Set your metabolite name
mtab_full_name <- "4m2op"
mtab_short_name <- "4m2op"
...
OD600_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/4m2op/4m2op_2/KHH01_106_4m2op_2_forR.csv"

culture_condition_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/4m2op/4m2op_2/4m2op_2_conditions_description.csv"
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "900 uM Pyr 0 uM 4m2op"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B10 and D11 are crashy in this condition
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 0 uM 4m2op"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B3 is crashy in this condition
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 150 uM 4m2op"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B7 is crashy in this condition
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 0 uM 4m2op"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B4, C2, E11 are crashy in this condition
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 150 uM 4m2op"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B6, F11 are crashy in this condition. E2 is crashy but pulls it back in. G10 is also weird, it kinda takes off. 
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "0 uM Pyr 150 uM 4m2op"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#No growth in this condition! As expected. See some noise creeping up in wells G6, C11, E7
```

#Remove edge effect-y columns
I'm removing row B for this analysis as well as columns 2 and 11 - I just see so many weird crashers etc in row B, and it fits the "remove inner ring" criteria
...
save_plots(p, "4m2op_corrected_gc.pdf", path_for_saving_plots)

p <- p + theme(legend.position =  "none")

save_plots(p, "4m2op_corrected_gc_noLegend.pdf", path_for_saving_plots)
```
...
Condition does have a significant effect on carrying capacity. There is no significant difference between conditions with 4m2op and conditions with the equivalent amount of pyruvate but no 4m2op (i.e., 0 µM pyr ± 4m2op, N.S.; 300 µM pyr ± 4m2op, N.S.; 600 µM pyr ± 4m2op, N.S)
...
save_plots(p, "4m2op_K_byCondition_boxplot_and_points.pdf", path_for_saving_plots)

p <- p + theme(legend.position = "none")

save_plots(p, "4m2op_K_byCondition_boxplot_and_points_noLegend.pdf", path_for_saving_plots)
```

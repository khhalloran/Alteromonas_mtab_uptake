---
title: "Kynurenine_Alteromonas_growth_reanalysis_2024_04_10"
author: "KHH"
date: "4/10/2024"
...
Analyze growth of Alteromonas macleodii on kynurenine ± pyruvate. 
...
Kynurenine also dissolves better under acidic conditions, so the kynurenine stock adds HCl to dissolve under mildly acidic conditions. Although the final concentration of HCl added to cultures was low, we tested the effect of HCl addition by including cultures in this experiment that received pyruvate and trace HCl (but no kynurenine) to ensure that HCl was not inhibiting growth. 
...
path_for_saving_plots <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/04.2024_reanalysis/kynurenine/Figures/"

  
##Set your metabolite name
mtab_full_name <- "Kynurenine"
mtab_short_name <- "Kyn"
...
OD600_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Kynurenine/KHH01_022_kynurenine_1_forR.csv"

culture_condition_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Kynurenine/gc_kynurenine_1_condition_descriptions.csv"
...
#omg this is all over the place. 
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "900 uM Pyr 0 uM Kyn"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#Oof this is all over the place. E8, F6, G7 are all much higher OD600 than B9, C10, D4, and I have no idea why. Yikes. Glad I got better at culture work, glad this experimental design has some redundancy. 
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 100 uM Kyn"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#This one looks beautiful. No complaints
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 0 uM Kyn"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#Also beautiful, no complaints
```


```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 100 uM Kyn"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#Pretty good! Maybe a little crashy in E7 and F5 but not bad
```


```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 0 uM Kyn  "), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#Absolutely no idea what happened with B10 but it's clearly wrong - shifted up by a lot - and needs to be removed. I don't think this is a "well crashed, remove the whole column" problem - I think there was something in the way, or some instrumental error. 
```


```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 0 uM Kyn trace HCl"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "0 uM Pyr 100 uM Kyn"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
I don't really have edge effect columns in this experiment! Dope. Don't know what to do about the 900 µM condition though. I do need to remove well B10
...
#Remove "trace HCl" wells
To my eye, it looks like trace HCl doesn't have a big effect on growth. But I'll test it statistically
...
Confirmed! Trace HCl didn't have a significant effect on carrying capacity. I can remove those conditions from analysis

```{r remove trace MeOH conditions from anaysis}
d_gc_blank_corr_no_neg <- d_gc_blank_corr_no_neg %>%
  filter(!grepl("HCl", Condition.name))
```
...
save_plots(p, "Kyn_corrected_gc.pdf", path_for_saving_plots)

p <- p + theme(legend.position =  "none")

save_plots(p, "Kyn_corrected_gc_noLegend.pdf", path_for_saving_plots)
```

#Iterate through wells and find the overall carrying capacity (max - min OD600)
I did this above when I was looking for an effect of adding methanol, but now I'll do it again after removing those "trace HCl" conditions (removed above)
...
Condition does have a significant effect on carrying capacity. There is no significant effect of adding Kynurenine (i.e., 0 µM pyr ± kyn, n.s.; 300 µM pyr ± kyn, n.s; 600 µM pyr ± kyn, n.s.)
...
p <- p + 
  ...
  geom_point() + 
   scale_color_manual(
     name = "Carbon source",
     breaks = c("Kyn", "Pyruvate", "Pyruvate and Kyn", "None"),
     labels = c(mtab_full_name, "Pyruvate", "Mix", "No carbon added"),
     values = c("firebrick", "#084594", "#4a1486", "gray")) + 
  theme_classic(base_size = 16)

save_plots(p, "Kyn_K_byCondition_boxplot_and_points.pdf", path_for_saving_plots)

p <- p + theme(legend.position = "none")

save_plots(p, "Kyn_K_byCondition_boxplot_and_points_noLegend.pdf", path_for_saving_plots)
```

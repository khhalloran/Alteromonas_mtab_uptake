---
title: "Leu_Alteromonas_growth_reanalysis_2024_04_10"
author: "KHH"
date: "4/10/2024"
...
Analyze growth of Alteromonas macleodii on leucine ± pyruvate. 
...
path_for_saving_plots <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/04.2024_reanalysis/leucine/Figures/"

  
##Set your metabolite name
mtab_full_name <- "leucine"
mtab_short_name <- "Leu"
...
OD600_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Leu/KHH01_095_leucine_forR.csv"

culture_condition_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Leu/Leu_condition_descriptions.csv"
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "900 uM Pyr 0 uM Leu"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 0 uM Leu"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 150 uM Leu"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B3 and C2 crash; G5 has a delayed crash
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 0 uM Leu"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#omg so many crashers. C11, G3, F2, B8. RIP. Not sure how I'll handle the stats here
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 M Pyr 150 uM Leu"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#D2, B6, G7 crashy. G7 not quite as bad as the others
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "0 uM Pyr 150 uM Leu"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B10, G6, E11 all questionable; E11 is the worst of these
```

#Remove edge effect-y columns
There is no way to remove all the edge effect columns/rows (I'd need to remove all 4) and still preserve an adequate sample size. I'm really not sure what to do. But for now I think I will remove all 4
...
This leaves ≥ 3 replicates in each condition except for condition D, 300 µM pyr 0 µM Leu. I don't know what I'd do with condition D regardless, since 4 wells in that condition really did crash. So I'm going to continue on with the data as is
...
save_plots(p, "Leu_corrected_gc.pdf", path_for_saving_plots)

p <- p + theme(legend.position =  "none")

save_plots(p, "Leu_corrected_gc_noLegend.pdf", path_for_saving_plots)
```
...
Condition does have a significant effect on carrying capacity. There is no significant difference between 600 µM pyr ± Leu
...
p <- p + 
  ...
  geom_point() + 
   scale_color_manual(
     name = "Carbon source",
     breaks = c("Leu", "Pyruvate", "Pyruvate and Leu", "None"),
     labels = c(mtab_full_name, "Pyruvate", "Mix", "No carbon added"),
     values = c("firebrick", "#084594", "#4a1486", "gray")) + 
  theme_classic(base_size = 16)

save_plots(p, "Leu_K_byCondition_boxplot_and_points.pdf", path_for_saving_plots)

p <- p + theme(legend.position = "none")

save_plots(p, "Leu_K_byCondition_boxplot_and_points_noLegend.pdf", path_for_saving_plots)
```

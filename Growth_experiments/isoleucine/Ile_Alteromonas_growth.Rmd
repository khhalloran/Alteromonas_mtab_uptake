...
Analyze growth of Alteromonas macleodii on isoleucine ± pyruvate. 
...
path_for_saving_plots <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/04.2024_reanalysis/isoleucine/Figures/"

  
##Set your metabolite name
mtab_full_name <- "isoleucine"
mtab_short_name <- "Ile"
...
OD600_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Ile/KHH01_111_isoleucine_forR.csv"

culture_condition_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Ile/Ile_condition_descriptions.csv"
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "900 uM Pyr"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#F2 takes a weird jump and G4 crashes (though not super early) 
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 0 uM Ile"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B3 and D2 crash, and G8 has a delayed crash
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 150 uM Ile"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 0 uM Ile"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#E2 crashy, B8 kinda weird in this condition
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 150 uM Ile"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#C11 and G3 crashy. B5 kinda weird but not exactly crashy
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "0 uM Pyr 150 uM Ile"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B6 very weird in this condition. G10 and D11 not great
```

#Remove edge effect-y columns
Removing all of B, G, 2, 11, still leaves me three replicates per condition - woo! I'm gonna do that. 
...
save_plots(p, "Ile_corrected_gc.pdf", path_for_saving_plots)

p <- p + theme(legend.position =  "none")

save_plots(p, "Ile_corrected_gc_noLegend.pdf", path_for_saving_plots)
```
...
Condition does have a significant effect on carrying capacity. There is no significant difference between 600 µM pyr ± Ile
...
p <- p + 
  ...
  geom_point() + 
   scale_color_manual(
     name = "Carbon source",
     breaks = c("Ile", "Pyruvate", "Pyruvate and Ile", "None"),
     labels = c(mtab_full_name, "Pyruvate", "Mix", "No carbon added"),
     values = c("firebrick", "#084594", "#4a1486", "gray")) + 
  theme_classic(base_size = 16)

save_plots(p, "Ile_K_byCondition_boxplot_and_points.pdf", path_for_saving_plots)

p <- p + theme(legend.position = "none")

save_plots(p, "Ile_K_byCondition_boxplot_and_points_noLegend.pdf", path_for_saving_plots)
```

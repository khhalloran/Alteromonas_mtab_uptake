---
title: "Val_Alteromonas_growth_reanalysis_2024_04_10"
author: "KHH"
date: "4/10/2024"
...
Analyze growth of Alteromonas macleodii on valine ± pyruvate. 
...
path_for_saving_plots <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/04.2024_reanalysis/valine/Figures/"

  
##Set your metabolite name
mtab_full_name <- "valine"
mtab_short_name <- "Val"
...
OD600_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Val/KHH01_114_valine_forR.csv"

culture_condition_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Val/Val_conditions_description.csv"
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "900 uM Pyr 0 uM Val"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#No big crashers but there's a very very weird couple of points at t = ~32.5h in well E3 w/ outlier OD600 (OD600 > 0.075). Should consider removing just those. Well G5 does see some weird secondary growth at the end. 
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 0 uM Val"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#No real crashers! Well G8 gets slightly odd at end
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 180 uM Val"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#Nothing too weird in this one!
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 0 uM Val"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B3 crashy in this condition
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 180 uM Val"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#D2 very weird in this condition. G7 a bit weird (but not crashy exactly)
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "0 uM Pyr 180 uM Val"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#B4 very weird in this condition
```

#Remove edge effect-y columns
For this analysis, removing row B, column 2. Going back and removing well E3, time = 32.0 and well E3, t = 32.5 from input data. 
...
save_plots(p, "Val_corrected_gc.pdf", path_for_saving_plots)

p <- p + theme(legend.position =  "none")

save_plots(p, "Val_corrected_gc_noLegend.pdf", path_for_saving_plots)
```
...
Condition does have a significant effect on carrying capacity. There is no significant difference between 300 µM pyr ± Val or 600 µM pyr ± Val
...
p <- p + 
  ...
  geom_point() + 
   scale_color_manual(
     name = "Carbon source",
     breaks = c("Val", "Pyruvate", "Pyruvate and Val", "None"),
     labels = c(mtab_full_name, "Pyruvate", "Mix", "No carbon added"),
     values = c("firebrick", "#084594", "#4a1486", "gray")) + 
  theme_classic(base_size = 16)

save_plots(p, "Val_K_byCondition_boxplot_and_points.pdf", path_for_saving_plots)

p <- p + theme(legend.position = "none")

save_plots(p, "Val_K_byCondition_boxplot_and_points_noLegend.pdf", path_for_saving_plots)
```

---
title: "Thymidine_Alteromonas_growth_reanalysis_2024_04_10"
author: "KHH"
date: "4/10/2024"
...
Analyze growth of Alteromonas macleodii on thymidine ± pyruvate. 
...
path_for_saving_plots <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/04.2024_reanalysis/thymidine/Figures/"

  
##Set your metabolite name
mtab_full_name <- "Thymidine"
mtab_short_name <- "Thy"
...
OD600_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Thymidine/thymidine addition 2/KHH01_020_thymidine_2_cat_forR.csv"

culture_condition_data <- "~/Documents/MIT_WHOI_JP/Kuj:Chisholm data/Growth curves/Metabolite addition/Thymidine/thymidine addition 2/gc_thymidine_condition_descriptions.csv"
...
Our experimental design included lower concentration metabolite additions, but we decided they did not contribute to our understanding of Alteromonas metabolite uptake, and so those conditions were not considered in the manuscript. 

```{r filter out the lowest C metabolite condition}
d_gc <- d_gc %>% 
  filter(Total_mM_C != 0.45) %>%
  filter(Total_mM_C != 0.60) #%>%
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "900 uM Pyr 0 uM thymidine"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 0 uM thymidine"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "600 uM Pyr 100 uM thymidine"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#Some weird behavior out of C7, but no crashing per se
```

```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 0 uM thymidine"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "300 uM Pyr 100 uM thymidine"), aes(x = time, y = OD600_corr, color = well, fill = well))
...
```{r}
p <- ggplot(data = filter(d_gc_blank_corr_no_neg, Condition.name == "0 uM Pyr 100 uM thymidine"), aes(x = time, y = OD600_corr, color = well, fill = well))

p + geom_point(size = 2)

#D3 is very weird and clearly not biological - remove from analysis
```

#Remove edge effect-y columns
Remove well D3 and column 11
...
This leaves ≥ 4 replicates/condition
...
save_plots(p, "Thy_corrected_gc.pdf", path_for_saving_plots)

p <- p + theme(legend.position =  "none")

save_plots(p, "Thy_corrected_gc_noLegend.pdf", path_for_saving_plots)
```
...
Condition does have a significant effect on carrying capacity. There is no significant difference between pyruvate concentrations ± thymidine (i.e., 0 pyr ± thy, n.s.; 300 µM pyr ± thy, n.s.; 600 µM pyr ± thy, n.s.)
...
p <- p + 
  ...
  geom_point() + 
   scale_color_manual(
     name = "Carbon source",
     breaks = c("Thy", "Pyr", "Pyr and Thy", "None"),
     labels = c(mtab_full_name, "Pyruvate", "Mix", "No carbon added"),
     values = c("firebrick", "#084594", "#4a1486", "gray")) + 
  theme_classic(base_size = 16)

save_plots(p, "Thy_K_byCondition_boxplot_and_points.pdf", path_for_saving_plots)

p <- p + theme(legend.position = "none")

save_plots(p, "Thy_K_byCondition_boxplot_and_points_noLegend.pdf", path_for_saving_plots)
```
